...
const initialStore = {
  ...
  navbar: {
    links: [
      // These objects define what should appear on the navbar
      {
        path: "/",
        text: "Exercises",
      },
...
const globalReducer = (globalState = initialStore.global, action) => {
  switch (action.type) {
    case actionNames.openOperation:
      return { ...globalState, openOperation: true };
    case actionNames.closeOperation:
      return { ...globalState, openOperation: false };
    ...
    case actionNames.deleteExerciseLocally: // Delete the exercise
      return {
        ...
        exercises: globalState.exercises.filter(
          (exercise) => !(exercise._id === action.exerciseId)
        ),
      };
...
const createExerciseViewReducer = (
  state = initialStore.createExerciseView,
  action
) => {
  switch (action.type) {
...
const editExerciseViewReducer = (
  state = initialStore.editExerciseView,
  action
) => {
  switch (action.type) {
